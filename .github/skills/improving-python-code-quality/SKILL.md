...
- Review Python library code before release.
- Introduce or tighten static analysis in an existing project.
- Refactor code to improve typing, readability, and maintainability.
- Standardize quality checks across contributors.

## Workflow

1. Establish baseline diagnostics.
2. Apply lint and format corrections with Ruff.
3. Fix typing gaps and run MyPy.
4. Refactor anti-patterns into Pythonic idioms.
5. Validate completion against the checklist.

## Decision Points

1. If `ruff check` reports import/order/style issues, fix those first and re-run until clean.
2. If MyPy fails due to missing annotations in public APIs, add type hints before internal/private typing improvements.
3. If strict MyPy settings create too much initial noise, start with targeted modules and expand coverage incrementally.
4. If refactoring changes behavior risk, add or update tests before continuing.
